...
def get_summoner_name_by_puuid(_region_code, puuid, api_key):
    ## Get a summoner by its RSO encrypted PUUID.
    # url = f"https://{region_code}.api.riotgames.com/fulfillment/v1/summoners/by-puuid/{puuid}?api_key={api_key}"
    ## Get a summoner by PUUID.
    status_code = 0
    while(status_code != 200):
        url = f"https://{region_code}.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/{puuid}?api_key={api_key}"
        response = requests.get(url)
        status_code = response.status_code
        if response.status_code == 200:
            summoner_data = json.loads(response.text)
...
series = pd.Series(df["id"])
puuids_name_pairs = []
cnt = 1
for puuid in series:
    try:
        puuids_name_pairs.append([puuid,get_summoner_name_by_puuid(region_code,puuid,api_key)])
    except:
        continue
    finally:
        print(f'{cnt}/37742')
...
asd = pd.DataFrame(puuids_name_pairs,columns=["puuid","summoner_name"])

asd.to_csv("summoners2.csv")
